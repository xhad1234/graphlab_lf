  ...
  private: 
    ...
    bool try_to_quit(size_t threadid,
      ...
      if (timer::approx_time_seconds() - engine_start_time > timed_termination) {
		...
		logstream(LOG_INFO)  << "Time out for :" << timed_termination <<",total time:"<<timer::approx_time_seconds() - engine_start_time<< std::endl;
        force_stop = true;
      }
      fiber_control::yield();
			...
			for(int i = 0; i < newvertexseqs.size(); i++ ) {
				if(newvertexseqs[i].valid){
					...
					if( newvertexseqs[i].index >= a_rcu.windex
						...
						unsigned aint = a; 
						logstream(LOG_INFO) << "locked :" <<aint<<" , record index:"<<newvertexseqs[i].index<<" z aindex:"<<a_rcu.windex<< std::endl;
						ok = false;
						break;
					}
					newvertexseqs[i].valid = false;
